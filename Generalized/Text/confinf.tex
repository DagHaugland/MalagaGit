...
\subsection{MILP formulations and valid inequalities for weakly confluent-influent flow} \label{sec:ciMILP}
Combining the flow variables $\boldsymbol{f}$ with binary variables gives a natural formulation of the weak MCCIFP.
...
\min & \sum_{(i,j)\in A}c_{ij}f_{ij} \label{eq:objMCCIFP} \\
     ...
     & x_p^u\in\{0,1\} & p\in P, u\in S_p\cup T_p \label{eq:x}
\end{eqnarray}
