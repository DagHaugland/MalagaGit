...
Consistency between $\boldsymbol{f}$ and $\boldsymbol{x}=\left(x_p^u\right)_{p\in P, u\in S_p\cup T_p}$ implies that for all arcs $(i,j)$
in $D(\boldsymbol{f})$ and all $s\in S_i$, it holds that $x_i^s=1$ (node $i$ receives flow only from source $s$) if $x_j^s=1$
...
An upper bound, $h_{ij}^{st}$, on this flow, is given as the maximum flow from $j$ to $i$ in the digraph $D_{ij}=(N,A_{ij})$,
where $A_{ij}=\left(A\cup\{(t,s)\}\right)\setminus\{(i,j)\}$, and the arc $(t,s)$ has capacity $h_{ts}=h_{ij}$.
% Define $b_u^{st}=b_u$ for $u\in\{s,t\}$, and $b_u^{st}=0$ for all $u\in (S\cup T)\setminus\{s,t\}$.
Let $\boldsymbol{f}=\left(f_{ij}^{st}\right)_{(i,j)\in A}$.
...
\min & \sum_{(i,j)\in A}c_{ij}f_{ij} \label{eq:objmMCCIFP} \\
     & f_{ij} = \sum_{s\in S}\sum_{t\in T}f_{ij}^{st} & (i,j)\in A \label{eq:fmMCCIFP} \\
     & f_{ij}^{st}\leq h_{ij}^{st}\left(x_i^s+x_i^t\right) & (i,j)\in A, s\in S_i, t\in T_j \label{eq:imMCCIFP} \\
     & f_{ij}^{st}\leq h_{ij}^{st}\left(x_j^s+x_j^t\right) & (i,j)\in A, s\in S_i, t\in T_j \label{eq:jmMCCIFP} \\
     & f_{ij}^{st}\leq h_{ij}^{st}\left(2-x_i^t-x_j^s\right) & (i,j)\in A, s\in S_i, t\in T_j \label{eq:ijmMCCIFP} \\
